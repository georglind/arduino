...
int opened;     // is the box open? (0 = closed, 1 = open)
...
{	
  ...
  opened = EEPROM.read(2);     // read open
  
  if (attempts==0 && opened==1) {
     puzzlelock(1);
  }
  ...
  if (opened==1 && solved==0)
  {
    ...
    EEPROM.write(2,0); opened = 0; // close box
  ...
  if (attempts > 50 && solved==0 && opened==0)
  { 
  ...
  // CLOSED AND UNSOLVED
  if (opened==0 && solved==0 && attempts < 50)
  {
